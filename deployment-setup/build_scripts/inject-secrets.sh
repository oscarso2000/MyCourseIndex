#!/bin/bash
mkdir ~/.kube
mv ./deployment-setup/build-scripts/kubeconfig ~/.kube/config
...
source ./deployment-setup/build-scripts/kube-secrets.txt
export $(cut -d= -f1 ./deployment-setup/build-scripts/kube-secrets.txt)
